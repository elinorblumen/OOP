...
/**
 * Class representing a "type" type sort object.
 *  * Sorts file names by their type.
 */
public class MergeSortType implements SortFiles{
    ...
    private void merge(String[] array, int l, int m, int r){
        ...
        while(i < n1 && j < n2){
            ...
            String fileLeftType  = getFileType(left[i]);
            String fileRightType = getFileType(right[j]);
            if(isReverseSuffix){
                if(fileLeftType.compareTo(fileRightType) > 0){
                    array[k] = left[i];
                    i++;
                }
                else if(fileLeftType.compareTo(fileRightType) < 0){
                    array[k] = right[j];
                    j++;
                }
                else{
    ...
    /**
     * Helper method that receives a file name and returns its type.
     * @param fileName The name of the file.
     * @return The type of the file of such exists or an empty string otherwise.
     */
